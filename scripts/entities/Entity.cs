...
public class Entity : KinematicBody2D, Collidable
{
	...
	protected List<Entity> _above = new List<Entity>();
	...
	public virtual void DoCollisionStuff(KinematicCollision2D touched)
	{
		...
		if (touched.Normal.y < -0.7)
		{
			_ground = true;
			Entity entity = touched.Collider as Entity ?? (touched.Collider as Node)?.GetParent() as Entity;
			if (entity != null) _above.Add(entity);
		}
